...
nPeople = info.nPeople # NUMBER OF PEOPLE SET IN DATA #
					   ...
					   ############################################
nAveraged = 10         # Number of last years taken into account  #
...
hSick = [r.TH1D("sick_" + info.cancerName[i],\
"sick_" + info.cancerName[i],NBINS, 0, 50) \
for i in xrange(0,info.nCancers+1)]

hDetected = [r.TH1D("detected_" + info.cancerName[i],\
"detected_" + info.cancerName[i],NBINS, 0, 50) \
for i in xrange(0,info.nCancers+1)]

hSurv = [r.TH1D("survived_" + info.cancerName[i],\
"survived_" + info.cancerName[i],NBINS, 0, 50) \
for i in xrange(0,info.nCancers+1)]
	...
	for j in xrange(1, info.nCancers+1):
		for i in xrange(firstSickYear, y[1].nBins):
			try:
				...
				c = info.diagnosTime[j].getBinContent(i) / Population[i-firstSickYear] * 10000.
				info.diagnosTime[j].setBinContent(i, c)

				a = info.nSurvival[j].getBinContent(i) / Population[i-firstSickYear] * 10000.
				info.nSurvival[j].setBinContent(i, a)
