...
MODE = 0               # PET impact:    0 - only background    ,#
                       ...
                       ###########################################
nPeople = 10**4        # Number of people in the initial cohorte #
					   ###########################################


					   ############################################
nAveraged = 20         # Number of last years taken into account  #
					   # while calculating average values    	  #
					   ############################################

					   ########################
nTry = 100			   # Number of iterations #
					   ########################
...
def calculate():
	...
	for j in xrange(1, len(info.nSick)):
		tempReturnList.append(info.nSick[j].avLast(nAveraged))
	retVal.append(cp.copy(tempReturnList))
	tempReturnList = []

	for j in xrange(1, len(info.diagnosTime)):
		tempReturnList.append(info.diagnosTime[j].avLast(nAveraged))
	retVal.append(cp.copy(tempReturnList))
	tempReturnList = []

	for j in xrange(1, len(info.nSurvival)):
		tempReturnList.append(info.nSurvival[j].avLast(nAveraged))
	retVal.append(cp.copy(tempReturnList))
	tempReturnList = []
...
dispHist = h.histo(nBins = 30)


for i in xrange(0,nTry):
	print "\nTRY NUMBER", i+1
	currResult = calculate()
	dispHist.fill(currResult[0][0])

dispHist.makeRootHist()
dispHist.draw()
